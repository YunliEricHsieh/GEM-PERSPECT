measure = read.csv("Data/Mutant_phenotypes_table_filtered_with_uniportIDs.csv", header = TRUE)

# remove the non-necessary column
measure <- measure[, !(names(measure) %in% c("Feature", "Mutant_ID", "Confidence_level", "Gene_Name"))]
...
write.csv(new_table, 'Data/condition_table_filtered.csv', row.names = F)
